    ...
    public void read(File file)
        ...
        try (Scanner sc = new Scanner(file)) {
            ...
            String[] requiredColumns = {
                    "Match ID", "Action", "Currency", "Amount", "Created at"
            };
            ...
            while (sc.hasNextLine()) {
                ...
                BigDecimal amount =
                        new BigDecimal(data[columnNrs.get("Amount")]);
                LocalDateTime date =
                        parseDate(data[columnNrs.get("Created at")]);

                if (data[columnNrs.get("Action")].contains("Bonus")
                        && !coin.equals(fiat)) {
                    transactionList.add(
                            new Transaction(
                                    date, coin, amount, null,
                                    BigDecimal.ZERO
                            )
                    );
            ...
            for (String match : matchOrders.keySet()) {
                ...
                LocalDateTime date = parseDate(
                        matchOrders.get(match)
                                .get(0)[columnNrs.get("Created at")]
                );
                ...
                transactionList.add(
                        new Transaction(
                                date, orderCoin, orderAmount, price, orderValue
                        )
                );
            }
        }
