    ...
    public void read(File file)
        ...
        try (Scanner sc = new Scanner(file)) {
            ...
            HashMap<String, Integer> columnNrs =
                    getColumnNrs(file, columns, requiredColumns);

            while (sc.hasNextLine()) {
                String[] data = sc.nextLine().split(",");
                LocalDateTime date =
                        LocalDateTime.parse(
                                data[columnNrs.get("Timestamp")],
                                DateTimeFormatter.ISO_DATE_TIME
                        );
                String coin = data[columnNrs.get("Asset")];
                String transactionType =
                        data[columnNrs.get("Transaction Type")];
                ...
                if (transactionType.equals("Buy")) {
                    ...
                    transactionList.add(
                            new Transaction(
                                    date, coin, amount, price, value.negate()
                            )
                    );
                    ...
                    transactionList.add(
                            new Transaction(
                                    date, coin, amount.negate(), price, value
                            )
                    );
                } else if (transactionType.equals("Learning Reward")) {
                    transactionList.add(
                            new Transaction(
                                    date, coin, amount, price, BigDecimal.ZERO
                            )
                    );
                    ...
                    String[] convertData =
                            data[columnNrs.get("Notes")].split(" ");
                    transactionList.add(
                            new Transaction(
                                    date, convertData[5],
                                    new BigDecimal(convertData[4]),
                                    null, value.negate()
                            )
                    );
                }
